export class Renderer {
        ...
        for (const selector of iter) {
            ...
            handlerOptionArr.forEach(({ eventName, handlerArr }) => {
                handlerArr.forEach((handler) => {
                    // event binding core
                    document.querySelector(selector).addEventListener(eventName, handler.bind(null, this.stateManager));
                });
            });
        }
        this.cnt++
    ...
    stateToTemplate() {
        ...
        document.querySelector('#inputText').innerText = stateManager.get('inputValue');
    }
}
