/**
 * store mapping relationship between element and event handlers
 */
export class EventManager {
  ...
  constructor(eventOptions) {
    Object.keys(eventOptions).forEach((key) => {
      this.addEventHandler(key, eventOptions[key]);
    });
  }
  ...
  addEventHandler(selector, handlerOptionArr) {
    ...
    handlerOptionArr.forEach((handlerOption) => {
      if (!handlerOption.eventName || !handlerOption.handlerArr) {
        throw new Error(
          "handlerOption should own both eventName and handler properties",
        );
      }
    });
